...
- `POST /api/progression/daily/claim`
- `POST /api/progression/missions/[id]/claim`

Profile and progression BFF routes:

- Profile and progression browser clients call local `/api/profile/*` and
  `/api/progression/*` routes through `apiFetch`.
- The local BFF routes proxy to backend `/api/v1/profile/*` and
  `/api/v1/progression/*` with `auth: true` through `backendFetch`.
- The avatar upload BFF route forwards multipart form data to backend
  `/api/v1/profile/avatar`; the confirmed multipart field name is `image`.
- Fetch helpers do not force `Content-Type: application/json` for `FormData`
  bodies so multipart boundaries are generated by fetch.
- Daily and mission reward claims return `balanceAfter` as a string; browser
  mutation hooks sync the auth-owned user balance from that backend value.
...
- Mission `type` and `status` enum values for progression responses. OpenAPI
  currently exposes those fields as generic objects, so frontend types keep
  them defensive.
