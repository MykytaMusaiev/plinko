...
export const PLINKO_ANIMATION_FILES: Record<number, string> = {
  ...
  15: 'fifteen-rows-animation.json',
  16: 'sixteen-rows-animation.json',
};
...
const libraryCache = new Map<number, Promise<PlinkoAnimationLibrary>>();

export function isSupportedPlinkoAnimationRows(rowsCount: number): boolean {
  return Object.hasOwn(PLINKO_ANIMATION_FILES, rowsCount);
}

export function loadPlinkoAnimationLibrary(rowsCount: number) {
...
export function preloadPlinkoAnimationLibrary(rowsCount: number) {
  return loadPlinkoAnimationLibrary(rowsCount);
}

async function fetchPlinkoAnimationLibrary(rowsCount: number) {
  const fileName = PLINKO_ANIMATION_FILES[rowsCount];

  if (!fileName) {
    throw new PlinkoAnimationAssetError(
      'canvas-animation-load-failed',
      `Plinko animation asset for ${rowsCount} rows is not supported.`,
    );
  }

  const response = await fetch(`/animations/plinko/${fileName}`);
...
function parsePlinkoAnimationLibrary(
  ...
  for (let bucketIndex = 0; bucketIndex <= rowsCount; bucketIndex += 1) {
    ...
    buckets.set(bucketIndex, variants.map((variant) => parsePath(variant)));
  }
