...
export function drawPlinkoBoard({
  ...
  for (const bucket of source.buckets) {
    drawBucket(
      context,
      bucket,
      multipliers[bucket.index] ?? 1,
      bucketFeedbacks.get(bucket.index),
      now,
...
function drawPeg(
  ...
  const progress =
    elapsed === null
      ? 0
      : Math.min(Math.max(elapsed / PLINKO_PEG_FEEDBACK_DURATION_MS, 0), 1);
  ...
  if (elapsed !== null && progress < 1) {
    ...
    if (secondaryAlpha > 0.01) {
      ...
      context.arc(
        peg.x,
        peg.y,
        peg.radius * (1 + secondaryProgress * 2.4),
        0,
        Math.PI * 2,
      );
...
function drawBucket(
  ...
  const style = getCanvasBucketStyle(bucket.index, bucketCount);
  const elapsed = startedAt === undefined ? null : now - startedAt;
  const progress =
    elapsed === null
      ? 0
      : Math.min(Math.max(elapsed / PLINKO_BUCKET_FEEDBACK_DURATION_MS, 0), 1);
  ...
  context.fillText(formatMultiplier(multiplier), bucket.centerX, centerY + 1);
  context.restore();
}
