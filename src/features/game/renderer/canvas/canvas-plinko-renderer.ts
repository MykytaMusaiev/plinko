...
const TURBO_REPLAY_SPEED_MULTIPLIER = 4.8;

export function createCanvasPlinkoRenderer({
  container,
  ...initialOptions
}: CreateCanvasPlinkoRendererOptions): PlinkoRenderer {
  ...
  function preloadSelectedRows() {
    ...
    void preloadPlinkoAnimationLibrary(rowsCount).catch(() => {
  ...
  function startPlayback(playback: PlaybackContext) {
    ...
    void loadPlinkoAnimationLibrary(playback.round.result.rowsCount)
      .then((library) => {
        ...
        if (trajectory.kind === 'failure' || !trajectory.path) {
  ...
  function drawFrameContents(now: number) {
    ...
    for (const active of activePlaybacks.values()) {
      ...
      const elapsedMs = Math.max(now - active.startedAt, 0) * active.replaySpeed;
    ...
    for (const visual of visiblePlaybacks) {
      drawPlinkoBall(canvasContext, visual.point, visual.active.layout.ballRadius);
    }
  ...
  function showBucketFeedback(round: PlinkoRendererRound): PlinkoRendererEnqueueResult {
    if (destroyed) {
      return {
        playbackId: null,
    ...
    const playback: PlaybackContext = {
      playbackId: `plinko-bucket-${++playbackSequence}-${round.id}`,
      retryCount: 0,
      round,
      selection: {
    ...
    const now = container.ownerDocument.defaultView?.performance.now() ?? 0;
